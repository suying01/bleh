...
import { PoseBuffer, recognizePose } from '@/lib/pose-recognizer'
...
export default function PoseGame({ stage, onBack }: PoseGameProps) {
    ...
    const poseBufferRef = useRef<PoseBuffer>(new PoseBuffer(30));
    ...
    const predictWebcam = useCallback(() => {
        if (!poseLandmarkerRef.current || !webcamRef.current?.video || !canvasRef.current) return;
            ...
            if (ctx) {
                ...
                if (results.landmarks) {
                    for (const landmarks of results.landmarks) {
                        ...
                        drawingUtils.drawConnectors(landmarks, PoseLandmarker.POSE_CONNECTIONS, { color: "#00f3ff", lineWidth: 2 });
                        drawingUtils.drawLandmarks(landmarks, { color: "#ff00ff", lineWidth: 1, radius: 3 });

                        // Dynamic Recognition
                        poseBufferRef.current.add(landmarks);
                        const dynamic = recognizePose(poseBufferRef.current);
                        if (dynamic) {
    ...
    const updateGame = useCallback((time: number) => {
        ...
        // Adaptive Spawn Rate (Slower)
        const baseSpawnRate = 1200 / stage.speedMultiplier; // Slower spawn (was 800)
        const spawnRate = Math.max(600, baseSpawnRate - (streak * 50));

        // Rapid Flow: If no active tiles, spawn immediately (after short buffer)
        const shouldSpawn = tiles.length === 0 && (spawnTimerRef.current > spawnRate || spawnTimerRef.current > 500); // Increased buffer to 500ms
    ...
    const startGame = async () => {
        ...
        setGameActive(false)
        setDetectedAction("NONE") // Clear any lingering action
        poseBufferRef.current.clear() // Clear gesture history
        ...
        <div ref={rootRef} className="relative w-full h-screen max-w-md mx-auto bg-black overflow-hidden flex flex-col font-sans select-none">
            ...
            <div className="relative z-10 p-4 flex justify-between items-start">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <div className="text-xs text-neon-blue font-bold">POSE POINTS</div>
                </div>
                <div className="w-10" />
            </div>
                ...
                {gameActive && tiles.map(tile => !tile.isHit && (
                    ...
                    >
                        ...
                        <div className="text-xl text-neon-pink font-bold animate-pulse">
                            {tile.char === "HAPPY" && "😊 RUB CHEST!"}
                            {tile.char === "HUNGRY" && "😋 RUB BELLY!"}
                            {tile.char === "COLD" && "🥶 HUG SELF!"}
                            {tile.char === "HOT" && "😓 CLAW FROM MOUTH!"}
                            {tile.char === "TIRED" && "😴 DROOP!"}
                        </div>
                    </motion.div>
                ))}
            ...
            {!gameActive && !countdown && !gameOver && (
                <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
                    <h1 className="text-5xl font-black text-white mb-2">POSE MODE</h1>
                    <p className="text-gray-400 mb-8 text-lg">Use your whole body!</p>

                    {isModelLoading ? (
                        <div className="flex flex-col items-center gap-4">
                            <Loader2 className="w-8 h-8 text-neon-blue animate-spin" />
                            <p className="text-white">Loading Pose Model...</p>
                        </div>
                    ) : (
