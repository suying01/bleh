...
export default function Home() {
  ...
  if (currentStage) {
    if (currentStage.id === 3) {
      return <PoseGame stage={currentStage} onBack={() => setCurrentStage(null)} />
    }
    if (currentStage.id === 2) {
      return <ActionGame stage={currentStage} onBack={() => setCurrentStage(null)} />
    }
    return <SignStreamGame stage={currentStage} onBack={() => setCurrentStage(null)} />
  }
